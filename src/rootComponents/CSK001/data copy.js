...
arr4_data.forEach((dataRow, rowIndex) => {
  ...
  arr1_configs.forEach((configItem, configIndex) => {
    ...
    // ⭐ STEP 0: Resolve mode & modeOBJ
    let resolvedMode = null;
    let modeObjData = null;

    // Resolve mode từ ADD_ → arr4
    ...
    // ⭐ Resolve modeOBJ CHỈ khi có resolvedMode
    // modeOBJ null → parseModeOBJ tạo {} + auto-id → tránh lỗi hoàn toàn
    if (resolvedMode) {
      let rawModeOBJ = null;
      if (configItem.modeOBJ && typeof configItem.modeOBJ === "string") {
        rawModeOBJ = parseSpecialValue(configItem.modeOBJ, dataRow);
      }
