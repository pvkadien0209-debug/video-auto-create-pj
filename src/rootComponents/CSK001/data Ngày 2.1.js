...
/**
 * ⭐⭐⭐ UPDATED: Group config keys by action number (skip null values)
...
function groupConfigsByAction(configItem) {
  ...
  // ⭐⭐⭐ Remove actions that have no content (styleCss alone is not enough)
    ...
    for (const [actionNum, actionData] of actionsMap) {
      ...
      if (process.env.NODE_ENV === "development") {
        console.log(`   └─ Action ${actionNum}:`, {
          contentName,
          styleCssName: styleCssName || "none",
        });
      }
      ...
      // Add styleCss if exists (optional)
      if (!isNullOrEmpty(styleCssName)) {
